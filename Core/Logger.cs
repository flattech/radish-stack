...
namespace Core
{
    public static class Logger
    {
        ...
        private static string getConfigFilePath()
        {
            ...
            string configPath = Path.Combine(basePath, "bin");
        ...
        public async static void SendTelegram(string message)
        {
            try
            {
                var bot = new TelegramBotClient("344201111:AAHxFc8YhpQhwvHHMpt5uqK_wFKxzhvYDdo");
                var t = await bot.SendTextMessageAsync("@radishstack", message);
            }
            catch (Exception ex)
            {
                //  Logger.Error(typeof(WebApiApplication), ex.Message);
            }
